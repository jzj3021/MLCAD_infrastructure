...
def run_flow(design_name, verilog_file="", score_file=""):
    ...
    ##### Test replacement ##############
    design.writeDef("gr1.def")
    fileDir = "./"    
    metrics = run_evaluation(design, fileDir+"gpl_results.csv")
    compute_score(score_file, *metrics)
    
    design.evalTclString("repair_design -verbose")
    design.writeDef("gr2.def")
    fileDir = "./"    
    ...
    design.evalTclString("global_placement -routability_driven -init_density_penalty 0.05 -initial_place_max_iter 10")
    design.evalTclString("estimate_parasitics -placement")
    design.writeDef("gr3.def")
    fileDir = "./"    
    metrics = run_evaluation(design, fileDir+"gr_results.csv")
    compute_score(score_file, *metrics)
    
    help(design)
    Insts = ord.get_db_block().getInsts()
    for inst in Insts:
      inst.setPlacementStatus("UNPLACED")
      if not inst.isPlaced():
        print("Inst %s is not placed, set placement status to UNPLACED"%inst.getName())
        sys.stdout.flush()
    design.evalTclString("global_placement -routability_driven -init_density_penalty 0.05 -initial_place_max_iter 10")
    design.evalTclString("estimate_parasitics -placement")
    design.writeDef("gr4.def")
    fileDir = "./"    
    metrics = run_evaluation(design, fileDir+"gr_results.csv")
    compute_score(score_file, *metrics)
    ####################################
...
def run_evaluation(design, fname):
    ...
    return leakage, tns, slew, cap, overflow, wns


def compute_score(score_file, leakage, tns, slew, cap, overflow, wns):
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", default="aes_cipher_top", help="Give the design name")
